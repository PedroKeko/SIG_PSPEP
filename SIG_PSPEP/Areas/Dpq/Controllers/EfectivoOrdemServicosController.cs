...
namespace SIG_PSPEP.Areas.Dpq.Controllers
{
    ...
    public class EfectivoOrdemServicosController(
    AppDbContext context,
    UserManager<IdentityUser> userManager,
    SignInManager<IdentityUser> signInManager,
    ILogger<EfectivoOrdemServicosController> logger) : BaseController(context)
    {
        private readonly ILogger<EfectivoOrdemServicosController> _logger = logger;
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    if (existePromocao)
                    {
                        ...
                        {
                            success = false,
                            message = "Já foi promovido com esta Ordem de Serviço."
                        });
                    }
                    ...
                    _context.EfectivoHistoricos.Add(new EfectivoHistorico
                    {
                        ...
                        Tipo = "Promoção",
                        Descricao = $"Promoção para a patente de {novaPatente?.Posto} via Ordem de Serviço Nº {ordemServico?.NumOrdemServico}",
                        Data = DateTime.Now,
                        Status = "Aprovado"
                    });
                    ...
                    {
                        success = true,
                        message = $"Promoção para {novaPatente.Posto} realizada com sucesso."
                    });
                }
                catch (Exception)
                {
                    await transaction.RollbackAsync();
                    return Json(new { success = false, message = "Erro interno ao processar a promoção." });
                }
            }
        ...
        public async Task<IActionResult> PromocaoMultipla([FromBody] List<EfectivoOrdemServicoDTO> promocoesDto)
        {
            ...
            if (promocoesDto == null || !promocoesDto.Any())
                return BadRequest(new { success = false, message = "Nenhuma promoção foi enviada." });
                    ...
                    try
                    {
                        ...
                        if (jaExiste)
                        {
                            erros.Add(new
                            {
                                ...
                                motivo = "Já promovido com esta Ordem de Serviço."
                            });
                            continue;
                        }
                        ...
                        if (efectivo.Patente != null && efectivo.Patente.grau >= novaPatente.grau)
                        {
                            erros.Add(new
                            {
                                ...
                                motivo = $"Patente nova ({novaPatente.Posto}) inferior ou igual à atual ({efectivo.Patente.Posto})."
                            });
                            continue;
                        }
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            ViewData["EfectivoId"] = new SelectList(_context.Efectivos, "Id", "Genero", efectivoOrdemServico.EfectivoId);
            ViewData["PatenteId"] = new SelectList(_context.Patentes, "Id", "Id", efectivoOrdemServico.PatenteId);
            ViewData["UserId"] = new SelectList(_context.Users, "Id", "Id", efectivoOrdemServico.UserId);
            return View(efectivoOrdemServico);
        }
        ...
        public async Task<IActionResult> Edit(int id, [Bind("EfectivoId,OrdemServicoId,PatenteId,NumDespacho,AnoPromocao,Id,Estado,DataRegisto,DataUltimaAlterecao,UserId")] EfectivoOrdemServico efectivoOrdemServico)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    _context.Update(efectivoOrdemServico);
                    await _context.SaveChangesAsync();
                }
                catch (DbUpdateConcurrencyException)
                {
                    if (!EfectivoOrdemServicoExists(efectivoOrdemServico.Id))
                    {
                        return NotFound();
                    }
                    else
